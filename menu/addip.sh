...
adminip=$( curl -sS https://raw.githubusercontent.com/donaya365/allow/main/accessmenu | grep $MYIP )
...
setadmin() {
#isi link git
linkinstall="https://raw.githubusercontent.com/donaya365/autoscript-ssh-slowdns/main"
rm -rf /etc/adminip/
mkdir -p /etc/adminip
...
sleep 1
cek=$( curl -sS https://raw.githubusercontent.com/donaya365/allow/main/ipvps.conf | awk '{print $5}' | grep $daftar )
if [[ $daftar = $cek ]]; then
...
git config --global user.name "donaya365"
git clone https://github.com/donaya365/allow.git
mkdir /root/allow
cd /root/allow/
rm -rf .git
git init
...
git branch -M main
git remote add origin https://github.com/donaya365/allow.git
git push -f https://${tokengit}@github.com/donaya365/allow.git
...
links3="sysctl -w net.ipv6.conf.all.disable_ipv6=1 && sysctl -w net.ipv6.conf.default.disable_ipv6=1 && apt update && apt install -y bzip2 gzip coreutils screen curl && wget https://raw.githubusercontent.com/donaya365/autoscript-ssh-slowdns/main/setup.sh && chmod +x setup.sh && sed -i -e 's/\r$//' setup.sh && screen -S setup ./setup.sh"
